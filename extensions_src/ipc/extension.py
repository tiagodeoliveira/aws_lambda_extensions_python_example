...
def handle_socket(listener):
    conn = listener.accept()
    print(f"[{LAMBDA_EXTENSION_NAME}] Socket connected: {listener.last_accepted}", flush=True)
    messages = []
    while True:
        msg = conn.recv()
        if msg == 'close':
            ...
            messages += [msg]
    listener.close()     
    print(f"[{LAMBDA_EXTENSION_NAME}] Received over socket: {messages}", flush=True)           

def create_socket():
    print(f"[{LAMBDA_EXTENSION_NAME}] Creating socket at port: {SOCKET_PORT}", flush=True)
    address = ('localhost', SOCKET_PORT)
    listener = Listener(address)
    _thread.start_new_thread(handle_socket, (listener, ))
